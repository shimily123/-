...
纯跟踪控制
'''
def pure_pursuit_control(init_state:VehicleState, path, ctrller:car_controllor):
    '''
    ...
    while ((state.x - path[last_index, 0])**2 + (state.y - path[last_index, 1])**2)**0.5 > 50:
...
if __name__ == '__main__':
    ...
    path_x = [0] * 10
    path_y = list(range(0, 2000, 200))
    path = np.array([path_x, path_y]).T

    # path_x = [0, 0, 0, 0, 0, 0, -300, -600, -900, -1200]
    # path_y = [0, 300, 600, 900, 1200, 1200, 1200, 1200, 1200, 1200]
    # path = np.array([path_x, path_y]).T
